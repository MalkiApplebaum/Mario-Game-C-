...
public:
	BoundedPhysicsDecorator(IPhysicsComponent);
	void test();
	// TODO:
	void reset(cv::Point const& tl) override;
	bool update(cv::Mat const& collisionMask) override;
	cv::Mat const& getCollisionMask() const override;
	bool checkCollision(std::shared_ptr<IPhysicsComponent> const& other) const override;
	cv::Point const& getTL() const override;
